...
module.exports = {
	...
	getSentimentForPartido: function (entity, callback) {
		var collection = db.collection('sentiment');
		collection.find({'partido': entity}).sort({'year': 1, 'month': 1}).toArray(function (err, doc) {
			if (err) {
				callback(null);
			}
			else {
	...
	getSentimentForInstitucion: function (entity, callback) {
		var collection = db.collection('sentiment');
		collection.find({'institucion': entity}).sort({'year': 1, 'month': 1}).toArray(function (err, doc) {
			if (err) {
				callback(null);
			}
			else {
